...
class ObjectStorage(LazyProxy[Any]):
    ...
    @robust
    async def put(
        self, *, key: str, file: UploadFile, bucket: str = "tera"
    ) -> UploadData:
        """
        ...
        url = await self.generate_presigned_url(key=key, bucket=bucket)
        return {"url": url, "key": key}

    @robust
    async def get(self, *, key: str, bucket: str = "tera") -> UploadData:
        """
